...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function performSearch(query, page) {
    if (!query) {
      searchResults.innerHTML = '<p class="text-muted">Inserisci un termine di ricerca.</p>';
      pagination.innerHTML = '';
      return;
    } else {
      searchResults.innerHTML = '<p class="text-muted">Caricamento...</p>';
    }
    fetch(`/wp-admin/admin-ajax.php?action=custom_news_search&term=${encodeURIComponent(query)}&page=${page}`).then(response => response.json()).then(data => {
      if (data.posts.length > 0) {
        const html = data.posts.map(post => `<li><a href="${post.link}"><h4>${post.title}</h4></a></li>`).join('');
        searchResults.innerHTML = `<ul>${html}</ul>`;
        updatePagination(data.current_page, data.max_num_pages);
      } else {
        searchResults.innerHTML = '<p class="text-muted">Nessun risultato trovato.</p>';
        pagination.innerHTML = '';
      }
    }).catch(() => {
