    ...
    fetch(`/wp-admin/admin-ajax.php?action=custom_news_search&term=${encodeURIComponent(query)}&page=${page}&post_type=${selectedPostType}`).then(response => response.json()).then(data => {
      ...
      if (data.posts.length > 0) {
        ...
        const htmlResult = data.posts.map(post => `
					...
					`).join('');
        searchResults.innerHTML = `<h5 class="mb-3">${selectedPostType.charAt(0).toUpperCase() + selectedPostType.slice(1).toLowerCase()} </h5>${htmlResult}`;
        const filterHtml = data.filtri.map(filter => {
          ...
          return `
							<li class="list-group-item ">
								<p class="${textColor} text-decoration-none fw-bold mb-0" data-post-type="${filter.name}" data-count="${filter.count}">
									${filter.name.charAt(0).toUpperCase() + filter.name.slice(1).toLowerCase()} (${filter.count})
								</p>
							</li>
						`;
        }).join('');
